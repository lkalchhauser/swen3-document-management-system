      ...
      <div className="library-header">
        ...
        <button className="upload-btn" onClick={() => setShowUpload(!showUpload)}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <path d="M8 1L8 11M8 1L11 4M8 1L5 4M2 14L14 14" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          Upload File
        </button>
      </div>
      ...
      <div className="documents-grid">
        {documents.length === 0 ? (
          ...
          documents.map(document => (
            <div key={document.id} className="document-item" onClick={() => window.location.href = `/document/${document.id}`}>
              <div className="document-icon">
                <div className="file-icon">
                  <div className="file-icon-bg">
                    {getFileIcon(document.metadata.contentType)}
                  </div>
                  <span className="file-type">{getFileTypeLabel(document.metadata.contentType, document.fileName)}</span>
                </div>
              </div>
              <div className="document-info">
                <h3 className="document-name">{document.fileName}</h3>
                <p className="document-date">{new Date(document.metadata.createdAt).toLocaleDateString()}</p>
              </div>
              <div className="document-actions" onClick={(e) => e.stopPropagation()}>
                <button className="action-btn" onClick={() => handleDeleteDocument(document.id)}>
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <path d="M6 2H10M2 4H14M12 4V12C12 13.1 11.1 14 10 14H6C4.9 14 4 13.1 4 12V4M6 6V11M10 6V11" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </button>
              </div>
            </div>
              ...
              <div className="upload-icon">
                {uploading ? (
                  ...
                  <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
                    <path d="M24 8L24 32M24 8L30 14M24 8L18 14M8 40L40 40" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                )}
              </div>
